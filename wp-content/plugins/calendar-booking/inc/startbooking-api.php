...
class StartBookingComAPI {
	...
	public function internal_proxy( $route, $params, $method, $duration = 0 ) {
		$key = md5( serialize( func_get_args() ) );
		if ( 'GET' == $method && $duration > 0 ) {
		...
		if ( 'GET' == $method && $duration > 0 && ! is_wp_error( $response ) ) {
			set_transient( $key, wp_remote_retrieve_body( $response ), $duration );
			$this->runtime_cache[ $key ] = wp_remote_retrieve_body( $response );
		}
...
function cbsb_api_request( $route, $params = array(), $method = 'GET', $duration = 0 ) {
